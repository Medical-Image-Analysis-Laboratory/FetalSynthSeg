...
# INPUT PATH (BIDS)
bids_path = Path('/media/vzalevskyi/data/fetal_dHCP/derivatives')
...
for sub in tqdm(subjects):
    for ses in sub.glob('ses-*'):
        ...
        try:
            imgs = list(sub.glob(f'{sesion}/anat/*_T2w.nii.gz'))[0]  # CHANGE THE PATTERN TO MATCH YOUR IMAGES
            label = list(sub.glob(f'{sesion}/anat/*drawem9_dseg.nii.gz'))[0]  # CHANGE THE PATTERN TO MATCH YOUR LABELS
